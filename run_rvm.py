...
for seq_path in seq_path_list:
    ...
    for output_path in output_path_list:
        ...
        frame_idx = int(osp.splitext(osp.basename(output_path))[0])
    ...
    frame_idx_list = sorted([int(osp.splitext(osp.basename(x))[0]) for x in glob(osp.join(save_root_path, '*.png'))])
